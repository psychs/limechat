...
- (id) initWithDelegate:(id)delegate userData:(NSNumber*)userData
{
  self = [super init];

  theFlags = 0x00;
  theDelegate = delegate;
  theUserData = [userData longValue];
  thePollTimer = nil;
...
- (NSNumber*) userData
{
  return [NSNumber numberWithLong:theUserData];
}

- (void) setUserData:(NSNumber*)userData
{
  theUserData = [userData longValue];
}
...
- (BOOL) acceptOnAddress:(NSString*)hostaddr port:(NSNumber*)aPort error:(NSError **)errPtr
{
  UInt16 port = [aPort shortValue];
...
- (BOOL) connectToHost:(NSString*)hostname onPort:(NSNumber*)aPort error:(NSError **)errPtr
{
  UInt16 port = [aPort shortValue];
    ...
    if ([theDelegate respondsToSelector:@selector(onSocket:didConnectToHost:port:)])
      [theDelegate onSocket:self
         didConnectToHost:[self addressHost:peer]
               port:[NSNumber numberWithInt:[self addressPort:peer]]];
    CFRelease (peer);
...
- (void) readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(NSNumber*)aTag
{
  long tag = [aTag longValue];
  if (length == 0) return;
...
- (void) completeCurrentRead
{
  ...
  {
    [theDelegate onSocket:self didReadData:theCurrentRead->buffer withTag:[NSNumber numberWithLong:theCurrentRead->tag]];
  }
...
- (void) writeData:(NSData*)data withTimeout:(NSTimeInterval)timeout tag:(NSNumber*)aTag;
{
  long tag = [aTag longValue];
  if (data == nil || [data length] == 0) return;
  if (theFlags & kForbidReadsWrites) return;
...
- (void) completeCurrentWrite
{
  ...
  if ([theDelegate respondsToSelector:@selector(onSocket:didWriteDataWithTag:)])
  {
    ...
    [theDelegate onSocket:self didWriteDataWithTag:[NSNumber numberWithLong:tag]];
  }
